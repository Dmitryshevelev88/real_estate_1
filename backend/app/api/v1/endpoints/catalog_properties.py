...
def search_catalog_properties(
    ...
    stmt = (
        ...
        .join(PropertyAnalytics, PropertyAnalytics.catalog_property_id == CatalogProperty.id)
        .where(
            CatalogProperty.is_active.is_(True),
            PropertyAnalytics.is_published.is_(True),
            or_(
                CatalogProperty.display_name.ilike(f"%{q}%"),
                CatalogProperty.address_full.ilike(f"%{q}%"),
                CatalogProperty.project_name.ilike(f"%{q}%"),
            ),
        )
